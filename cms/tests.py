...
class CommonTest(APITestCase):
    def setUp(self):
        User.objects.create_user(
            ...
            password='pass', is_staff=True, is_admin=True)
        User.objects.create_user(
            first_name='john', last_name='doe', email='regular@example.com',
            password='pass')
...
class ViewSetTests(CommonTest):
    ...
    def setUp(self):
        ...
        self.product_1 = Item.objects.create(
            ...
            uploadedImg='https://res.cloudinary.com/dgroams94/image/upload/v1/media/28316f3a-2f8_lsndfs'
        )
        self.product_2 = Item.objects.create(
            title='product title two',
            description='product description two',
            uploadedImg='https://res.cloudinary.com/dgroams94/image/upload/v1/media/9455c9f5-454_kr7i5f'
        )

    def test_can_retrieve_item_using_valid_id(self):
        """Test that a user can retrieve a product using a valid ID"""
    ...
    def test_cant_retrieve_item_using_invalid_id(self):
        """Test that a user can't retrieve a product using an invalid ID"""
        response = self.client.get('/products/999/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_user_cant_post_to_add_product(self):
        """Test that a user can't add a product"""
    ...
    def test_anonymous_user_can_submit_order(self):
        ...
        response = self.client.post(
            '/order/', self.get_order_data(), format='json')
        count = Order.objects.count()
        self.assertEqual(count, 1)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_authenticated_user_can_submit_order_and_save_address(self):
        """Test that an authenticated user can submit an order
        and save shipping address """
    ...
    def test_delete_bulk_product_success(self):
        ...
        response = self.client.delete(
            '/products/bulk_delete/', ids, format='json')
